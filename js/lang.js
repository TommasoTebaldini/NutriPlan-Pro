...
// ═══════════════════════════════════════════════════
// LABEL & CONTENT TRANSLATIONS (text-matching) — caricate pigramente
// ═══════════════════════════════════════════════════
// LABEL_TR (traduzioni EN/DE per testo libero non coperto da data-i18n) viveva
// prima inline qui: era il 73% delle dimensioni del file ma serve SOLO quando
// la lingua attiva non è italiano (il caso di stragrande maggioranza dei
// dietisti). Spostato in js/lang-labels.js, caricato on-demand solo se serve
// davvero - ottimizzazione performance 2026-07-30, vedi commit.
let _labelTrPromise = null;
function ensureLabelTr() {
  if (typeof LABEL_TR !== 'undefined') return Promise.resolve();
  if (_labelTrPromise) return _labelTrPromise;
  _labelTrPromise = new Promise((resolve, reject) => {
    const s = document.createElement('script');
    s.src = 'js/lang-labels.min.js';
    s.onload = resolve;
    s.onerror = reject;
    document.head.appendChild(s);
  });
  return _labelTrPromise;
}
