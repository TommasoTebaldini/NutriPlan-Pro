...
const SUPABASE_KEY = 'sb_publishable_ayv54WJGyaiA5XPSF-uNGw_zt6gxVwB';
let sb;
try {
...
async function loadProfile() {
  ...
  // Specialized sections additionally require explicit admin grant via sections_enabled.
  // ───────────────────────────────────────────────────────────────────────────
  if (!isAdmin) {
    const _SECTION_MAP = {
    ...
    // Gate page content if accessed directly without permission (specialized only)
    ...
    if (_pageKey && !allowed.includes(_pageKey)) {
      document.body.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100vh;flex-direction:column;gap:16px;font-family:inherit">
        ...
        <h2 style="margin:0;color:#1e293b">Sezione Specialistica</h2>
        <p style="color:#64748b;margin:0;text-align:center">L'accesso a questa sezione deve essere abilitato dall'amministratore.</p>
        <a href="linee-guida.html" style="color:#64748b;font-size:13px">← Torna alle linee guida</a>
      </div>`;
    }

    // Hide subscription badge (no free/pro distinction)
    const sbSub = document.getElementById('sb-subscription');
    if (sbSub) sbSub.style.display = 'none';
  }

  // Always hide "Abbonamento" nav link (subscription page not needed in sidebar)
